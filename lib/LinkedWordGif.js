...
class WordNode {
    ...
    addNeighbor(words) {
        if (this.i < words.length - 1) {
            this.next = new WordNode(this.i + 1, words)
            this.next.prev = this
        }
    }

    draw(context) {
        const tSize = context.measureText(this.word).width
        const sc1 = Math.min(0.5, this.state.scale) * 2
        const sc2 = Math.min(0.5, Math.max(0, this.state.scale - 0.5)) * 2
        const x = w/2 - tSize / 2
        //console.log(x)
        context.save()
        context.translate(x * sc1, h/2 + (h / 2 + Math.min(w, h) / 20) * sc2)
        context.fillStyle = 'white'
        context.font = context.font.replace(/\d{2}/, `${Math.min(w, h) / 20}`)
        // context.fillRect(0, 0, tSize, Math.min(w, h) / 20)
    ...
    update(cb) {
        //console.log(`${this.word} : ${this.state.scale}`)
